...
public class SortingTest extends BaseDatabaseTest {
	...
	public void columnSort() throws Exception {
		...
		final String[] sortedNatural = new String[] { "ID", "FIRSTNAME", "LASTNAME", "ADDRESS1", "ADDRESS2", "CITY",
	...
	public void fkSort() throws Exception {
		...
		final String[] sortedNatural = new String[] { "Z_FK_AUTHOR", "SYS_FK_10128", };
	...
	public void indexSort() throws Exception {
		...
		final String[] sortedNatural = new String[] { "SYS_IDX_PK_AUTHORS_10111", "IDX_B_AUTHORS", "IDX_A_AUTHORS", };
		final String[] sortedAlpha = Arrays.copyOf(sortedNatural, sortedNatural.length);
		Arrays.sort(sortedAlpha);
	...
	private void checkColumnSort(final String tableName, final String[] expectedValues,
		...
		if (table.getName().equals(tableName)) {
			...
			assertEquals("Column count does not match", expectedValues.length, columns.length);
			for (int i = 0; i < columns.length; i++) {
				final Column column = columns[i];
				assertEquals("Columns not " + (sortAlphabetically ? "alphabetically" : "naturally") + " sorted",
						expectedValues[i], column.getName());
			}
		}
	}
	...
	private void checkFkSort(final String tableName, final String[] expectedValues, final boolean sortAlphabetically)
		...
		for (final Table table : tables) {
			if (table.getName().equals(tableName)) {
				...
				assertEquals("Foreign key count does not match", expectedValues.length, foreignKeys.length);
				for (int i = 0; i < foreignKeys.length; i++) {
					final ForeignKey foreignKey = foreignKeys[i];
					assertEquals(
							"Foreign keys not " + (sortAlphabetically ? "alphabetically" : "naturally") + " sorted",
							expectedValues[i], foreignKey.getName());
				}
			}
		}
	}
	...
	private void checkIndexSort(final String tableName, final String[] expectedValues, final boolean sortAlphabetically)
		...
		for (final Table table : tables) {
			if (table.getName().equals(tableName)) {
				...
				assertEquals("Index count does not match for table " + table, expectedValues.length, indexes.length);
				for (int i = 0; i < indexes.length; i++) {
					final Index index = indexes[i];
					assertEquals("Indexes not " + (sortAlphabetically ? "alphabetically" : "naturally")
							+ " sorted  for table " + table, expectedValues[i], index.getName());
				}
			}
		}
	}
