...
public final class TestUtility {
	...
	private final static Pattern[] neuters = { Pattern.compile("url +jdbc:.*"),
			Pattern.compile("database product version.*"), Pattern.compile("driver version.*"),
			Pattern.compile("\\s+<schemaCrawlerVersion>.*"), Pattern.compile("\\s+<schemaCrawlerAbout>.*"),
			Pattern.compile("\\s+<schemaCrawlerInfo>.*"),
			Pattern.compile(".*[A-Za-z]+ \\d+\\, 201[456] \\d+:\\d+ [AP]M.*"),
			Pattern.compile(".*201[6-8]-\\d\\d-\\d\\d \\d\\d:\\d\\d:\\d\\d.*"),
			Pattern.compile(".*201[6-8]-\\d\\d-\\d\\dT\\d\\d:\\d\\d:\\d\\d\\.\\d\\d\\d.*"),
			// Apache Derby unnamed
			// database objects
			Pattern.compile("SQL\\d+\\s+\\[primary key\\]"),
			Pattern.compile("SQL\\d+\\s+\\[foreign key, with no action\\]"),
			// SVG {
			Pattern.compile("<svg.*"), Pattern.compile(" viewBox=\"0.00.*"), Pattern.compile("<g id=.*"),
			Pattern.compile("<text text-anchor.*"), Pattern.compile("<path fill=.*"),
			Pattern.compile("<ellipse fill=.*"), Pattern.compile("<polyline fill=.*"),
			Pattern.compile("<polygon fill=.*"), Pattern.compile(".*&#xd;"), };

	public static void clean(final String dirname) throws Exception {
		FileUtils.deleteDirectory(buildDirectory().resolve("unit_tests_results_output").resolve(dirname).toFile());
	}

	public static List<String> compareCompressedOutput(final String referenceFile, final Path testOutputTempFile,
	...
	public static Reader readerForResource(final String resource, final Charset encoding) throws IOException {
	...
	private static boolean contentEquals(final Reader expectedInputReader, final Reader actualInputReader,
			final Pattern... ignoreLinePatterns) throws Exception {
		...
		while (stackTraceIterator.hasNext()) {
			...
			if (stackTraceElement.getClassName().equals(TestUtility.class.getName())) {
				while (stackTraceIterator.hasNext()) {
					...
					if (!className.equals(TestUtility.class.getName()) && !className.equals(TestWriter.class.getName())
	...
	private static Reader openNewCompressedInputReader(final InputStream inputStream, final Charset charset)
			throws IOException {
	...
	private static Reader readerForFile(final Path testOutputTempFile, final boolean isCompressed) throws IOException {
	...
	private static Reader readerForResource(final String resource, final Charset encoding, final boolean isCompressed)
			throws IOException {
	...
	private static boolean validateJSON(final Path testOutputFile, final List<String> failures)
	...
	private static Path writeToTempFile(final InputStream resourceStream) throws IOException, FileNotFoundException {
		final Path tempFile = IOUtility.createTempFilePath("resource", "data").normalize().toAbsolutePath();
