...
public class SchemaCrawlerSystemTest extends AbstractSchemaCrawlerSystemTest {

	@Test
	public void schemaCounts() throws Exception {
		final int[] schemaCounts = { 65, 19, 16, 6, 5, 12, 1, };
		...
		for (int i = 0; i < dataSources.length; i++) {
			...
			assertEquals("Incorrect number of schemas for " + dataSource + ": " + Arrays.toString(schemas),
					schemaCounts[i], schemas.length);
		}
	}

	@Test
	public void tablesAndCounts() throws Exception {
	...
	private void counts(final String dataSourceName, final Schema schema, final Catalog catalog) throws Exception {

		final int[] tableColumnCounts = { 9, 3, 3, 6, 1, 2 };

		final int[] fkCounts = { 1, 0, 2, 1, 0, 0 };

		final Table[] tables = catalog.getTables(schema).toArray(new Table[0]);
		assertEquals(dataSourceName + " table count does not match", tableColumnCounts.length, tables.length);
		for (int tableIdx = 0; tableIdx < tables.length; tableIdx++) {
			final Table table = tables[tableIdx];
			assertEquals(String.format("%s table %s columns count does not match", dataSourceName, table.getFullName()),
					tableColumnCounts[tableIdx], table.getColumns().size());
			assertEquals(
					String.format("%s table %s foreign key count does not match", dataSourceName, table.getFullName()),
					fkCounts[tableIdx], table.getForeignKeys().size());
		}
	}
	...
	private Schema retrieveSchema(final String schemaInclusion, final Catalog catalog) throws Exception {
		...
		if (schemas == null || schemas.length == 0) {
			...
			final Pattern schemaPattern = Pattern.compile(".*books", Pattern.CASE_INSENSITIVE);
	...
	private Schema retrieveSchema(final String dataSourceName, final String schemaInclusion) throws Exception {
		final Catalog catalog = retrieveDatabase(dataSourceName, schemaInclusion);
		return retrieveSchema(schemaInclusion, catalog);
	}

	private void tables(final String dataSourceName, final Schema schema, final String quote, final Catalog catalog)
			throws Exception {
		requireNonNull(schema, "No schema found");

		final String[] tableNames = { "AUTHORS", "AUTHORSLIST", "BOOKAUTHORS", "BOOKS", quote + "GLOBAL COUNTS" + quote,
				"PUBLISHERS", };
		final String[] tableTypes = { "TABLE", "VIEW", "TABLE", "TABLE", "TABLE", "TABLE" };

		final Table[] tables = catalog.getTables(schema).toArray(new Table[0]);
		assertEquals(dataSourceName + " table count does not match - " + ObjectToString.toString(tables),
				tableNames.length, tables.length);
		for (int tableIdx = 0; tableIdx < tables.length; tableIdx++) {
			final Table table = tables[tableIdx];
			assertEquals(dataSourceName + " table name does not match", tableNames[tableIdx],
					table.getName().toUpperCase());
			assertEquals(dataSourceName + " table type does not match", tableTypes[tableIdx], table.getTableType());
		}
	}

	private String tablesAndCounts(final String dataSourceName, final String schemaInclusion, final String quote) {
		try {
			final Catalog catalog = retrieveDatabase(dataSourceName, schemaInclusion);
